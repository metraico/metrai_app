...
def render_simulation_screen(backend_url: str):
    ...
    with tab_simulate:
        ...
        with cfg_col:
            ...
            smoothing_days = st.number_input("Demand Smoothing Window (days)", min_value=7, max_value=90, value=_default_smoothing)

            with st.expander("Store Config"):
                store_reorder_weeks = st.slider("Min Inventory Trigger (weeks of cover)", 1, 4, 2, 1)
                store_target_weeks  = st.slider("Store Target Stock (weeks)",             1, 8, 3, 1)
                store_start_days    = st.number_input("Store Starting Stock (days)", min_value=1, max_value=60, value=14)
                store_order_dow     = st.selectbox("Store Order Day", ["MONDAY","TUESDAY","WEDNESDAY","THURSDAY","FRIDAY"], index=0)

            with st.expander("DC Config"):
                dc_reorder_weeks = st.slider("DC Reorder Point (weeks of cover)", 1, 6, 2, 1)
                dc_target_weeks  = st.slider("DC Target Stock (weeks)",           2, 12, 5, 1)
                dc_start_days    = st.number_input("DC Starting Stock (days)", min_value=1, max_value=90, value=30)
                dc_review_dow    = st.selectbox("DC Review Day", ["MONDAY","TUESDAY","WEDNESDAY","THURSDAY","FRIDAY"], index=0)

            with st.expander("Supplier Config"):
                sup_lead_min = st.number_input("Lead Time Min (days)", min_value=1, max_value=14, value=3)
                sup_lead_max = st.number_input("Lead Time Max (days)", min_value=1, max_value=30, value=7)
            ...
            with st.expander("DC → Store Config"):
                dc_lead_days = st.number_input("DC → Store Lead Time (days)", min_value=1, max_value=14, value=2)
                dc_on_time   = st.slider("DC On-Time Rate (global)", 0.5, 1.0, 0.95, 0.05)
                dc_partial   = st.slider("DC Partial Delivery Rate (global)", 0.0, 0.3, 0.05, 0.05)
            ...
            if _dc_list:
                with st.expander(f"Per-DC Rates & Lead Time ({len(_dc_list)} DCs)"):
