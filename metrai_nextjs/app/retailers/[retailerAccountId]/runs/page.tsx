    ...
    <div className="w-full px-6 py-6">
      <div className="w-full">
        <div className="mb-5 flex items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-black tracking-tight text-charcoal-blue-950">Simulation Runs</h1>
            <p className="mt-0.5 text-xs font-medium text-charcoal-blue-400">View and manage your simulation runs</p>
          </div>
        ...
        {!loading && !error && runs.length === 0 && (
          <div className="rounded-xl border border-charcoal-blue-200 bg-white p-12 text-center shadow-sm">
            ...
            <button onClick={handleNew}
              className="mt-5 inline-flex items-center gap-1.5 rounded-xl bg-majorelle-blue-500 px-4 py-2 text-xs font-bold text-white transition-all hover:bg-majorelle-blue-600">
            ...
            {runs.map((run) => (
              ...
              >
                <button
                  ...
                  className="absolute right-3 top-3 rounded-lg p-1 text-charcoal-blue-300 transition hover:bg-rose-50 hover:text-rose-500"
                >
                  <Trash2 size={13} />
                </button>

                <div className="mb-3">
                  <span className={`inline-block rounded-lg px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${STATUS_STYLES[run.simulation_status] ?? 'bg-charcoal-blue-100 text-charcoal-blue-600'}`}>
